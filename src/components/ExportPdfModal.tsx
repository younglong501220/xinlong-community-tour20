...
export const ExportPdfModal: React.FC<ExportPdfModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-primary/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-4xl bg-surface-container-lowest rounded-2xl shadow-2xl overflow-hidden border border-outline-variant/30 max-h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="bg-primary text-on-primary px-space-lg py-space-md flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-secondary text-on-secondary text-sm font-semibold hover:bg-secondary/90 transition-colors cursor-pointer"
            >
        ...
        <div className="p-space-xl overflow-y-auto flex-1 bg-white text-on-surface">
          {/* Printable Document Header */}
          <div className="border-b-2 border-primary pb-space-md flex items-center justify-between">
          ...
          {/* Quick Summary Grid */}
          <div className="grid grid-cols-3 gap-space-md my-space-md p-space-md bg-surface-container-low rounded-lg text-xs">
            <div>
          ...
          <div className="my-space-md">
            <h2 className="text-sm font-bold text-primary border-l-4 border-secondary pl-2 mb-2">
              當日行程規劃 (09:30 - 17:00)
            </h2>
            <div className="border border-outline-variant/40 rounded-lg overflow-hidden text-xs">
              <table className="w-full text-left">
                <thead className="bg-surface-container-low font-semibold text-primary">
                  <tr>
                    <th className="p-2 w-28">時間</th>
                    <th className="p-2 w-48">體驗項目</th>
                    <th className="p-2">核心內容</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant/30">
                  {ITINERARY_STOPS.map(stop => (
                    <tr key={stop.id}>
                      <td className="p-2 font-mono font-bold text-secondary">{stop.time}</td>
                      <td className="p-2 font-semibold text-primary">{stop.title}</td>
                      <td className="p-2 text-on-surface-variant">{stop.description}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Banquet Menu */}
          <div className="my-space-md">
            <h2 className="text-sm font-bold text-primary border-l-4 border-tertiary pl-2 mb-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {BANQUET_MENU.map(dish => (
                <div
                  key={dish.id}
                  className="p-2 border border-outline-variant/30 rounded bg-surface-container-lowest flex justify-between items-center"
                >
          ...
          {/* Notice and Weather */}
          <div className="my-space-md p-space-sm bg-surface-container-low rounded-lg text-xs text-on-surface-variant flex flex-col gap-1">
          ...
          {/* Footer Contact */}
          <div className="pt-space-sm border-t border-outline-variant/30 text-xs text-on-surface-variant flex justify-between items-center">
        ...
        <div className="bg-surface-container-low px-space-lg py-space-xs flex justify-between items-center shrink-0 border-t border-outline-variant/20">
